...
- Chapter content edits follow [`skills/<chapter>/SKILL.md`](../skills/README.md) and the matching op (`populate` / `audit` / `improve`). Do not fork skills per brand. Task routing: [`docs/agent/workflow.md`](../docs/agent/workflow.md).
- Shell vs brand extension points: [`UPSTREAM.md`](../UPSTREAM.md).

- Shell chrome is Astryx (`AppShell` + `SideNav` in `src/components/AppShell.tsx`). Do not reintroduce custom `.app-shell` CSS.
- **Modes:** Define is `/` (guide TOC + chapter hash nav). Create is `/create` (workspace scaffold). Switch via `ModeSwitch` in the SideNav header. Shell-owned — not a brand.md surface.
...
- **Typefaces (N faces):** Design system owns the CSS stacks (`--font-sans` required → `brandThemeInput.fontSans` → body / `--font-family-body`; optional `--font-serif` → heading / `--font-family-heading`; optional `--font-mono` → `--font-family-mono`). Visual fields **Type display.** / **Type primary.** / **Type mono.** (+ foundries) drive one TypefaceSection each; hide display/label leaves when empty. `layout.tsx` owns Next `next/font` loaders and injected CSS variables on `<html>` — every stack `var(--font-*)` must have a matching `variable:` here (required edit, not a human note). Do not codegen `next/font` from stack strings; do not apply a separate `next/font` className on `body`. Live UI reads Astryx `--font-family-*`, not `--font-sans` / `--font-serif` / `--font-mono` directly. Recipe: [`skills/typography/SKILL.md`](../skills/typography/SKILL.md).
- **Adaptive leaves (N-of-whatever):** Runtime nav = `GUIDE_NAV` catalog ∩ authored data ∩ extended-on (`filterNavForSetup` → `withApplicationsFromExpressions` → `filterNavForTypeFaces` → `filterNavForAuthoredLeaves`). One Expressions row = one Applications leaf; Phrases / We Say / photo categories / color proportion hide when empty. Sample Brand is the sparse case, not the maximum. Map: [`docs/agent/section-inventory-spec.md`](../docs/agent/section-inventory-spec.md).
