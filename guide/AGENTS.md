...
- **Typefaces (two owners):** Design system owns the CSS stacks (`--font-sans` required → `brandThemeInput.fontSans` → body; optional `--font-serif` → heading). `layout.tsx` owns Next `next/font` loaders and injected CSS variables (today `--font-geist-sans`). Keep those variables inside the authored stacks. Do not codegen `next/font` from stack strings; do not apply a separate `next/font` className on `body` (that fights the theme stack). Live UI reads Astryx `--font-family-*`, not `--font-sans` / `--font-serif` directly.
